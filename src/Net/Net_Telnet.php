...
class Net_Telnet
{
            ...
            if (is_array($opts)) {
                ...
                if (array_key_exists('port', $opts)) {
                    if (is_numeric($opts['port']) && $opts['port'] > 0)
                        ...
                        if ($p = getservbyname($opts['port'], 'tcp'))
                            $this->port = $p;
                        else
                ...
                if (array_key_exists('connect', $opts)) {
                    ...
                    $this->debug(
                            "auto_connect set " . ($auto_connect ? "on" : "off"));
                }
                ...
                if (array_key_exists('pager', $opts)) {
                    ...
                    $this->debug(
                            "pager set " . ($this->mode['pager'] ? "on" : "off"));
                }
            }
    ...
    function connect ($opts = null)
    {
        ...
        if ($this->s !== null)
            if (! $this->disconnect())
                throw new Exception(
                        "connect called with non-null socket and disconnect failed");
        
        if (ip2long($this->host)) {
            $this->debug(
                    "attempting connection to " . $this->host . ":" . $this->port);
            $this->s = fsockopen($this->host, $this->port, $errno, $errstr, 
                    $this->timeout);
            if ($this->s)
            ...
            if ($addrs = gethostbynamel($this->host)) {
                foreach ($addrs as $a) {
                    ...
                    $this->debug(
                            "attempting connection to " . $a . ":" . $this->port);
                    $this->s = fsockopen($a, $this->port, $errno, $errstr, 
                            $this->timeout);
                    if ($this->s)
                ...
                throw new Exception(
                        "invalid or unknown hostname: " . $this->host);
        }
    ...
    function send_telcmd ($cmd = self::TEL_NOP, $opt = null, $data = null)
    {
        ...
        switch ($cmd) {
            ...
            case self::TEL_SB:
                ...
                $this->put_data(
                        self::TEL_IAC . self::TEL_SB . $opt . $data .
                                 self::TEL_IAC . self::TEL_SE, false, false);
                break;
            case self::TEL_SE:
            ...
            case self::TEL_WILL:
                ...
                switch ($opt) {
                    case self::TELOPT_BINARY:
                        ...
                        if (array_key_exists(self::TEL_DO, 
                                $this->telcmds['sent'][$opt])) {
                    ...
                    case self::TELOPT_SGA:
                        ...
                        if ($this->mode['telnet_bugs'] && ! $this->mode['tx_sga']) {
                            ...
                            if (! array_key_exists(self::TEL_WILL, 
                                    $this->telcmds['sent'][$opt]))
            ...
            case self::TEL_WONT:
                ...
                switch ($opt) {
                    case self::TELOPT_BINARY:
                        ...
                        if (! array_key_exists(self::TEL_DONT, 
                                $this->telcmds['sent'][$opt]))
                            $this->send_telcmd(self::TEL_DONT, $opt);
                        break;
                    case self::TELOPT_ECHO:
                        ...
                        switch ($this->mode['echomode']) {
                            case "remote":
                                $this->debug(
                                        "Remote won't ECHO, performing Local Echo");
                                $this->mode['echo_local'] = true;
                                break;
                        }
                    ...
                    case self::TELOPT_SGA:
                        ...
                        $this->debug(
                                "Disabling Suppress Go Ahead (SGA) on Receive");
                        ...
                        if ($this->mode['telnet_bugs'] && $this->mode['tx_sga']) {
                            $this->debug(
                                    "Disabling Suppress Go Ahead (SGA) on Transmit" .
                                             " (workaround for broken TELNETs)");
                            $this->mode['tx_sga'] = false;
                            
                            if (! array_key_exists(self::TEL_WONT, 
                                    $this->telcmds['sent'][$opt]))
                                $this->send_telcmd(self::TEL_WONT, $opt);
                        }
    ...
    function cmd ($arg)
    {
        ...
        foreach ($cmds as $cmd) {
            if (! $ok) {
                $this->debug(
                        "cmd: aborting command {$cmd} due to previous failure");
                continue;
            }
            $this->println($cmd);
            if (($ret = $this->waitfor($this->prompt)) === false) {
                $this->debug(
                        "cmd: waitfor({$this->prompt}) failed," .
    ...
    function expect ($arg, $arg2 = null)
    {
        ...
        if (array_key_exists($this->lastmatch, $args)) {
            $this->debug(
                    "expect: read_stream found {$this->lastmatch}," .
                             " sending {$args[$this->lastmatch]}");
            return $this->send($args[$this->lastmatch]);
        } else
            throw new Exception(
                    "expect broke, don't have lastmatch ({$this->lastmatch})");
    }
        ...
        while (! $found && ! feof($this->s) &&
            ...
            if ($this->mode['telnet'] && $c == self::TEL_IAC) { /*
                ...
                if (! feof($s) && ($c = fgetc($s)) === false) {
                    ...
                    if ($info['eof'])
                        ...
                        if ($info['timed_out'])
                            ...
                            if ($this->mode['telnet_bugs']) {
                                $this->debug(
                                        "Error reading TELNET command char from network");
                                break;
                            } else
                                throw new Exception(
                                        "Error reading TELNET command char from network");
                ...
                switch ($c) {
                        ...
                        if ($c === false) {
                            ...
                            if ($info['eof'])
                                ...
                                if ($info['timed_out'])
                                    ...
                                    if ($this->mode['telnet_bugs']) {
                                        $this->debug(
                                                "Error reading TELNET SubNegotiation command");
                                        continue;
                                    } else
                                        throw new Exception(
                                                "Error reading TELNET SubNegotiation command");
                    ...
                    case self::TEL_AO:
                        $this->debug(
                                "Received TELNET Abort Output (AO), clearing read buffers");
                        $this->userbuf = '';
                        $buf = '';
                        break;
                    case self::TEL_IP:
                        throw new Exception(
                                "Received TELNET Interrupt Process (IP)");
                        break;
                    case self::TEL_NOP:
                    default:
                        if (self::TELCMD_OK($c))
                ...
                if ($this->mode['telnet']) {
                    ...
                    if (ord($c) > 127 && (! $this->mode['rx_binmode'])) {
                        $this->debug(
                                "discarding non-ASCII char (" . ord($c) . ")");
                        continue;
                    }
                }
                ...
                if ($this->mode['pager'] && (strlen($this->page_prompt) > 0) && (substr(
                        $buf, 0 - strlen($this->page_prompt)) ===
                         $this->page_prompt)) {
    ...
    function login ($args = null, $arg2 = null)
    {
        if (is_string($args)) {
            ...
            if (is_array($args)) {
                ...
                if (array_key_exists('password_prompt', $args)) {
                    ...
                    $this->debug(
                            "password_prompt set to " . $args['password_prompt']);
                }
                
                if (array_key_exists('login_success', $args)) {
                    $this->login['login_success'] = $args['login_success'];
                    $this->debug(
                            "login_success set to " . $args['login_success']);
                }
        ...
        if ((! array_key_exists('login_success', $this->login)) ||
            ...
            $this->debug(
                    "login_success defaulting to " . $this->login['login_prompt']);
    ...
    $this->debug(
            "login: waiting for login prompt:  " . $this->login['login_prompt']);
...
$this->debug(
    "login: waiting for password prompt:  " . $this->login['password_prompt']);
...
if (($ret = $this->waitfor($this->prompt)) === false)
throw new Exception(
        "login: error in telnet session," . " didn't find prompt (failed login?)");
} else
$ret = $this->get_data();
